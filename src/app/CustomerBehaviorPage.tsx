...
import { fetchChannelPreference, fetchChannelTrend, fetchRepeatProducts, type ChannelPreference, type ChannelTrend, type RepeatProduct } from "@/lib/api";
      ...
      <div className="grid gap-6 grid-cols-1 xl:grid-cols-2">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="xl:col-span-2">
          <Card className="glass-card">
            ...
            <CardContent>
              {loading ? <div className="h-64 flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-500" /></div> : (
                <div className="h-64">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Card className="glass-card h-full">
            <CardHeader><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-indigo-500" />Channel Preference by Region</CardTitle></CardHeader>
            <CardContent>
              {loading ? <div className="h-56 flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-500" /></div> : (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <Card className="glass-card h-full">
            <CardHeader><CardTitle className="flex items-center gap-2"><RefreshCw className="h-5 w-5 text-purple-500" />High-Purchase Products</CardTitle></CardHeader>
            <CardContent>
              {loading ? <div className="h-56 flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-500" /></div> : (
                <div className="space-y-3">
                  {repeats.length === 0 ? (
                    <p className="text-slate-400 text-sm text-center py-8">No repeat purchase data yet — add more sales records.</p>
                  ) : repeats.map((r, i) => (
