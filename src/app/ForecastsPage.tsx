...
export function ForecastsPage() {
  ...
  const chartData = Array.from(
    forecasts.reduce((map, f) => {
      const row = map.get(f.forecast_month) ?? { month: f.forecast_month };
      row[f.category] = Number(f.predicted_revenue);
            ...
            <Card className="glass-card">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <TrendingUp className="h-5 w-5 text-indigo-500" />Predicted Revenue by Category
                </CardTitle>
              </CardHeader>
              <CardContent>
                {loading
                  ? <div className="h-72 flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-500" /></div>
                  : (
                    <div className="h-72">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                          ...
                          <Legend />
                          {categories.map((cat, i) => (
                            <Area key={cat} type="monotone" dataKey={cat} stroke={COLORS[i % COLORS.length]}
                        ...
                        <tr key={i} className="border-b border-slate-100 dark:border-slate-800 hover:bg-white/40 dark:hover:bg-white/5 transition-colors">
                          ...
                          <td className="py-2 px-3 text-right">
                            <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium
                              ${Number(f.confidence) > 70 ? "bg-green-100 text-green-700" : Number(f.confidence) > 40 ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}>
                              {f.confidence ?? "—"}%
                            </span>
                          </td>
                        </tr>
