...
import { Loader2, Globe, Store, TrendingUp } from "lucide-react";
import { fetchChannelRevenue, fetchCategoryPerformance, fetchRegionalDemand, type ChannelRevenue, type CategoryPerformance, type RegionalDemand } from "@/lib/api";
            ...
            <Card className="glass-card">
              <CardContent className="p-6 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900 dark:text-white">${Number(ch.revenue).toLocaleString()}</p>
                  <p className="text-xs text-slate-400">{ch.orders} orders · avg ${Number(ch.avg_order_value).toFixed(0)}</p>
                </div>
      ...
      <div className="grid gap-4 md:gap-8 grid-cols-1 xl:grid-cols-2">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}>
          <Card className="glass-card h-full">
            ...
            <CardContent>
              {loading ? <div className="h-60 flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-500" /></div> : (
