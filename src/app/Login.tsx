...
import { BarChart3, Mail, Lock, User, ArrowRight, Loader2, Github, CheckCircle2, Phone, Key } from "lucide-react";
        ...
        <div className="w-full max-w-md space-y-8">
          <div className="text-center lg:text-left">
            <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white mb-2">
              {isSignUp ? "Create an account" : isReset ? "Reset password" : "Welcome back"}
            </h1>
            <p className="text-slate-500 dark:text-slate-400">
              {isSignUp ? "Enter your details to get started." : isReset ? "Reset via phone authentication." : "Sign in to your OmniSales dashboard."}
            </p>
          </div>
          ...
          {!isReset && (
            <>
              <div className="flex flex-col gap-3">
                <Button
                  variant="outline"
                  className="w-full h-11 justify-center gap-2 font-medium bg-slate-900 text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200"
                  type="button"
              ...
              <AnimatePresence mode="wait">
                {isLoading
                  ...
                  : <motion.div key="label" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-2 font-medium">
                      {isSignUp ? "Create account" : mode === "reset_phone" ? "Send OTP" : mode === "reset_verify" ? "Reset & Sign In" : "Sign In"} <ArrowRight className="h-4 w-4" />
                    </motion.div>
                }
              </AnimatePresence>
            </Button>
          ...
          <p className="text-center text-sm text-slate-500 dark:text-slate-400 flex flex-col gap-2">
            {!isReset ? (
              <span>
