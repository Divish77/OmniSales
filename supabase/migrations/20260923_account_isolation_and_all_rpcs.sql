-- ============================================================
-- FULL MULTI-TENANCY & ALL RPCs MIGRATION
-- Account Isolation + Complete RPC Suite
-- ============================================================

-- 1. Ensure user_id column exists & backfill unassigned rows to divishbansal77@gmail.com
UPDATE harmonized_sales 
SET user_id = '94172c46-ec30-43f9-ad2d-fcb5cdcfee9c'::uuid 
WHERE user_id IS NULL;

-- 2. Dynamic Drop of all RPC functions to avoid PGRST203 overloads
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN 
        ...
        WHERE n.nspname = 'public' 
          AND p.proname IN (
            'get_channel_trend', 'get_basket_analysis', 'get_loyalty_signals',
            'get_channel_kpis', 'get_repeat_products', 'get_strategic_briefing',
            'get_analytics_insights', 'get_sales_kpis', 'get_mom_growth',
            'get_top_regions', 'get_forecasts_v2', 'get_dynamic_ml_insights',
            'get_ai_insights_v2', 'get_ai_recommendations', 'get_channel_revenue',
            'get_category_performance', 'get_total_revenue', 'get_monthly_sales',
            'get_category_distribution', 'get_top_products', 'get_filter_options',
            'generate_ai_insights_engine', 'generate_analytics_insights_engine',
            'generate_forecast_engine'
          )
    LOOP
...
-- Helper macro concept for tenant filter:
-- user_id = COALESCE(auth.uid(), '94172c46-ec30-43f9-ad2d-fcb5cdcfee9c'::uuid)

-- ============================================================
-- 1. get_channel_revenue
-- ============================================================
...
LANGUAGE sql SECURITY DEFINER AS $$
    ...
    WHERE (user_id = COALESCE(auth.uid(), '94172c46-ec30-43f9-ad2d-fcb5cdcfee9c'::uuid))
...
LANGUAGE sql SECURITY DEFINER AS $$
    SELECT
        ...
        STRING_AGG(DISTINCT channel, '/' ORDER BY channel) AS channel_split
    FROM harmonized_sales
    WHERE (user_id = COALESCE(auth.uid(), '94172c46-ec30-43f9-ad2d-fcb5cdcfee9c'::uuid))
      AND (p_country      IS NULL OR country            = p_country)
      AND (p_region       IS NULL OR region              = p_region)
...
DECLARE
  ...
  uid       uuid;
BEGIN
  uid := COALESCE(auth.uid(), '94172c46-ec30-43f9-ad2d-fcb5cdcfee9c'::uuid);
  ...
  WHERE user_id = uid AND sale_date >= eff_start AND sale_date <= eff_end
    AND (p_country  IS NULL OR country            = p_country)
  ...
  WHERE user_id = uid AND sale_date >= (eff_start - (eff_end - eff_start + 1)) AND sale_date < eff_start
    AND (p_country  IS NULL OR country            = p_country)
    AND (p_state    IS NULL OR region              = p_state)
    AND (p_product  IS NULL OR normalized_product = p_product)
    AND (p_category IS NULL OR category           = p_category);
...
-- 13. get_mom_growth
-- ============================================================
CREATE OR REPLACE FUNCTION get_mom_growth(
    p_country      text DEFAULT NULL,
    p_state        text DEFAULT NULL,
    p_product      text DEFAULT NULL,
    p_category     text DEFAULT NULL,
...
LANGUAGE sql SECURITY DEFINER AS $$
  ...
  WHERE (h.user_id = COALESCE(auth.uid(), '94172c46-ec30-43f9-ad2d-fcb5cdcfee9c'::uuid))
...
BEGIN
  ...
  WHERE user_id = uid AND (p_country IS NULL OR country=p_country)
...
BEGIN
  ...
  SELECT json_agg(DISTINCT country) INTO v_countries FROM harmonized_sales WHERE user_id = uid AND country IS NOT NULL;
  SELECT json_agg(DISTINCT region) INTO v_states FROM harmonized_sales WHERE user_id = uid AND region IS NOT NULL;
  SELECT json_agg(DISTINCT normalized_product) INTO v_products FROM harmonized_sales WHERE user_id = uid AND normalized_product IS NOT NULL;
  SELECT json_agg(DISTINCT category) INTO v_categories FROM harmonized_sales WHERE user_id = uid AND category IS NOT NULL;
...
-- ============================================================
-- 22. No-op Engine Trigger Functions for AddSalePage
-- ============================================================
CREATE OR REPLACE FUNCTION generate_ai_insights_engine() RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN END; $$;
CREATE OR REPLACE FUNCTION generate_analytics_insights_engine() RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN END; $$;
CREATE OR REPLACE FUNCTION generate_forecast_engine() RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN END; $$;
